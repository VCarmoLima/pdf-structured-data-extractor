...
caminho_pdf = r"caminho_do_pdf"

def mapear_coordenadas():
    with pdfplumber.open(caminho_pdf) as pdf:
        pagina = pdf.pages[0] 

        print(f"--- Dimensões da Página: Largura {pagina.width} x Altura {pagina.height} ---")
        print("Imprimindo as primeiras 20 palavras e suas posições para exemplo:\n")
        palavras = pagina.extract_words()
        for i, palavra in enumerate(palavras):
            if i > 20: break
            print(f"Texto: '{palavra['text']}' | Coordenadas: x0={palavra['x0']:.2f}, top={palavra['top']:.2f}, x1={palavra['x1']:.2f}, bottom={palavra['bottom']:.2f}")
...
def criar_gabarito(caminho_pdf):
    ...
    with pdfplumber.open(caminho_pdf) as pdf:
        ...
        for x in range(0, int(largura_pdf), passo):
            ...
            draw.line([(pos_pixel, 0), (pos_pixel, altura_img)], fill=cor, width=largura)
        ...
        for y in range(0, int(altura_pdf), passo):
            ...
            draw.line([(0, pos_pixel), (largura_img, pos_pixel)], fill=cor, width=largura)
